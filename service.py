...
try:
    ADDON = xbmcaddon.Addon('script.geetvguide')
    if ADDON.getSetting('cache.data.on.xbmc.startup') == 'true':
        Service()
    if ADDON.getSetting('autostart') == "true":
        xbmc.executebuiltin("RunAddon(script.geetvguide)")
except source.SourceNotConfiguredException:
    pass  # ignore
except Exception, ex:
    xbmc.log('[script.geetvguide] Uncaught exception in service.py: %s' % str(ex) , xbmc.LOGDEBUG)