...
#
##	Modified for GeeTV Guide. Added some fuzzy channel matching
##
##
...
class StreamsService(object):
    def __init__(self, addon):
        if (int(addon.getSetting('addons.ini.type')) == 0):
            ...
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'script.geetvguide', 'addons.ini'))
        else:
        ...
        for id in self.getAddons():
            ...
            for (label, stream) in self.getAddonStreams(id):
                if label == channel.title:
                    ...
                    title = re.sub(re.compile(r'uk$',flags=re.I),'',title)
